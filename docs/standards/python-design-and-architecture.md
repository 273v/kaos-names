...
These standards apply to Python code in `kaos-names`.

`kaos-names` publishes the `kaos_names` import package. This package publishes CLI entry point(s): `kaos-names`.

## Package Shape

- Keep the import package name aligned with the distribution name:
  `kaos-names` publishes import package `kaos_names`.
- Declare the public API in `kaos_names.__all__` when the package has
...
- Names exported from `kaos_names.__all__`.
...
- Put optional integrations in named extras. No public extras are currently declared.
